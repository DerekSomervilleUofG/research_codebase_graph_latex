...
def generate_average(number_of_repository):
    ...
    latex_repo += developer_graph(get_base_file_name(FILE_NAME), commit_package_average, "packages", 0, "Commit", "", param_caption=GRAPH_CAPTION.format(component="packages"))
    latex_repo += developer_graph(get_base_file_name(FILE_NAME), commit_class_average, "classes", 0, "Commit", "", param_caption=GRAPH_CAPTION.format(component="classes"))
    latex_repo += developer_graph(get_base_file_name(FILE_NAME), commit_method_average, "methods", 0, "Commit", "", param_caption=GRAPH_CAPTION.format(component="methods"))
    latex_repo += "    \\caption{" + FIGURE_CAPTION.format(freq="commit",commit=len(commit_package_average), number_of_repository=word_engine.number_to_words(number_of_repository)) +"} \n"
    latex_repo += latex_end_graph()
    latex_repo += latex_start_graph()
    latex_repo += developer_graph(get_base_file_name(FILE_NAME), daily_package_average, "packages", 0, "Daily", "", param_caption="New packages.")
    latex_repo += developer_graph(get_base_file_name(FILE_NAME), daily_class_average, "classes", 0, "Daily", "", param_caption="New classes.")
    latex_repo += developer_graph(get_base_file_name(FILE_NAME), daily_method_average, "methods", 0, "Daily", "", param_caption="New methods.")
