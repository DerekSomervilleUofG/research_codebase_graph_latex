...
def generate_data(commits, developers, developers_start, start_known, from_month = 0, to_month = 11):
    global max_commits, developer_total_commit, max_total_known
    template = [[1],[],[],[1],[1],[],[1],[],[1],[], "" ]
    developer_id = 0
    period_touched = {NUMBER_OF_WEEKS:[], NUMBER_OF_MONTHS:[], TIME_SERIES_NUMBER_OF_COMMIT:[], str(NUMBER_OF_WEEKS) + PERIOD:0, str(NUMBER_OF_MONTHS) + PERIOD:0}
    previous_day = 1
    ...
    for row in commits:
        ...
        if temp_developer_id > 0 and row[PACKAGE_KNOWN_COL] is not None:
            ...
            if developer_id in developers:
                ...
                number_of_months = math.ceil((previous_day/ 365) * NUMBER_OF_MONTHS)
                if number_of_months >= from_month and to_month >= number_of_months:
